...
fn block_change_icon(diff: &TagFieldDiff) -> Option<(ButtonIcon, &'static str)> {
    if diff.a.is_empty() && diff.base_path.is_none() && diff.b.starts_with("added — ") {
        Some((ButtonIcon::Add, "Block added in comparison tag"))
    } else if diff.b.is_empty() && diff.base_path.is_some() && diff.a.starts_with("removed — ") {
        Some((ButtonIcon::Remove, "Block removed in comparison tag"))
    } else {
        None
    }
}
...
impl Baboon {
    ...
    pub(super) fn draw_tag_diff_window(&mut self, ctx: &egui::Context) {
        ...
        let current_label = if state.source == TagCompareSource::EditingKit {
            self.kits[diff_kit]
                .profile
                .as_ref()
                .map(|profile| profile.name.clone())
        ...
        egui::Window::new("Compare Tags")
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("tag_compare_source_grid")
                    ...
                    .spacing([14.0, 10.0])
                    .show(ui, |ui| {
                        ui.label(RichText::new("Current Tag:").strong());
                        path_label(
                            ui,
                        ...
                        egui::ComboBox::from_id_salt("tag_compare_source")
                            ...
                            .show_ui(ui, |ui| {
                                for (source, label) in [
                                    ...
                                    let changed = ui
                                        .selectable_value(&mut state.source, source, text)
                                        .changed();
                                    if changed {
                                        state.results = None;
                                        state.error = None;
                    ...
                    if results.diffs.is_empty() {
                        ...
                        ui.horizontal(|ui| {
                            icon_text_dropdown_button(ui, ButtonIcon::Filter, "Filter", |ui| {
                                ui.checkbox(&mut state.filters.both, "Both tags");
                                ui.checkbox(&mut state.filters.current_only, "Current tag only");
                                ui.checkbox(
                                    &mut state.filters.comparison_only,
                                    "Comparison tag only",
                                );
                            });
                            let visible: Vec<&TagFieldDiff> = results
                                .diffs
                                .iter()
                                .filter(|diff| show_diff(state.filters, diff))
                                .collect();
                            let count = if visible.len() == results.diffs.len() {
                                format!("{} differing field(s)", visible.len())
                            } else {
                                format!(
                                    "{} of {} differing field(s)",
                                    visible.len(),
                                    results.diffs.len()
                                )
                            };
                                ...
                                .clicked()
                            {
                                let text = std::iter::once(format!(
                                    "field\t{current_label}\t{comparison_label}"
                                ))
                                .chain(
                                    visible
                                        .iter()
                        ...
                        ui.scope(|ui| {
                            ...
                            TableBuilder::new(ui)
                                ...
                                .column(Column::exact(96.0).resizable(false).clip(true))
                                .column(Column::remainder().at_least(80.0).clip(true))
                                .header(25.0, |mut header| {
                                    for title in ["Field", &current_label] {
                                        header.col(|ui| {
                                            ui.add_space(8.0);
                                            ui.add(
                                                egui::Label::new(
                                                    RichText::new(title).font(bold_font(14.0)),
                                                )
                                                .truncate()
                                    ...
                                    header.col(|ui| {
                                        ...
                                        ui.vertical_centered(|ui| {
                                            ui.label(RichText::new("Δ").font(bold_font(14.0)));
                                        });
                                    });
                                    header.col(|ui| {
                                        ui.add_space(8.0);
                                        ui.add(
                                            egui::Label::new(
                                                RichText::new(&comparison_label)
                                                    .font(bold_font(14.0)),
                                            )
                                            .truncate()
                                            .halign(egui::Align::Min),
                                        )
                                        .on_hover_text(&comparison_label);
                                    });
                                })
                                .body(|body| {
                                    body.rows(24.0, visible.len(), |mut row| {
                                        ...
                                        row.col(|ui| {
                                            ...
                                            if let Some((icon, tooltip)) = block_change_icon(diff) {
                                                ...
                                                numeric_delta(diff)
                                            {
                                                let (arrow, color, description) = match direction {
                                                    Ordering::Greater => (
                                                        "▲",
                                                        good_news(),
                                                        "Increased in comparison tag",
                                                    ),
                                                    Ordering::Less => (
                                                        "▼",
                                                        material_delete_text(),
                                                        "Decreased in comparison tag",
                                                    ),
        ...
        if compare {
            ...
            if let (Some(a), Some(b)) = (a, b) {
                ...
                let (diffs, truncated) = diff_tags(&a.tag, b, &names, 5000);
